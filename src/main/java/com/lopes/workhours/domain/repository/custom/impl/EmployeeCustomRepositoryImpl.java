...
public class EmployeeCustomRepositoryImpl implements EmployeeCustomRepository {
    ...
    private String mountWhere(EmployeeFilter filter) {
        ...
        if (hasText(filter.getUser())) {
            where.append("""
                    AND (
                            CAST(UNACCENT(UPPER(user.name)) AS STRING) LIKE CAST(UNACCENT(UPPER(CONCAT('%', :user, '%'))) AS STRING)
                            OR
                            CAST(UNACCENT(UPPER(user.nickName)) AS STRING) LIKE CAST(UNACCENT(UPPER(CONCAT('%', :user, '%'))) AS STRING)
                        )
                    """);
        }
